...
// Pricing Plans - Public endpoint for main website
app.get('/api/public/pricing-plans', async (req, res) => {
    try {
        console.log('Public pricing plans API called');
        const plans = await db.getPricingPlans();
        console.log('Pricing plans retrieved:', plans.length);
        res.json(plans);
    } catch (error) {
        console.error('Error in public pricing plans API:', error);
        res.status(500).json({ error: 'Failed to get pricing plans', details: error.message });
    }
});
