...
class AdminPanel {
    ...
    async saveNewPricingPlan() {
        ...
        try {
            // Show loading state
            const submitBtn = document.querySelector('#add-pricing-form button[type="submit"]');
            const originalText = submitBtn.textContent;
            submitBtn.textContent = 'Saving...';
            submitBtn.disabled = true;
            ...
            if (response && response.ok) {
                ...
                // Update local data
                this.data.pricing.push(createdPlan);
            ...
            const submitBtn = document.querySelector('#add-pricing-form button[type="submit"]');
            if (submitBtn) {
                submitBtn.textContent = 'Add Pricing Plan';
                submitBtn.disabled = false;
            }
        }
    }
    ...
    async savePricingEdit() {
        ...
        try {
            // Show loading state
            const submitBtn = document.querySelector('#edit-pricing-form button[type="submit"]');
            const originalText = submitBtn.textContent;
            submitBtn.textContent = 'Saving...';
            submitBtn.disabled = true;
            ...
            // Update pricing plan via API
            const response = await this.requestMiddleware(`/api/pricing-plans/${plan.id}`, {
                method: 'PUT',
                body: JSON.stringify({
                    title,
                    price,
            ...
            if (response && response.ok) {
                ...
                // Update local data
                const index = this.data.pricing.findIndex(p => p.id === plan.id);
                if (index !== -1) {
                    this.data.pricing[index] = updatedPlan;
                }
    ...
    async deletePricingPlan(id) {
        if (confirm('Are you sure you want to delete this pricing plan?')) {
            try {
                ...
                const response = await this.requestMiddleware(`/api/pricing-plans/${planId}`, {
                    method: 'DELETE'
                });
                ...
                if (response && response.ok) {
                    ...
                    this.showNotification('Failed to delete pricing plan', 'error');
                }
            } catch (error) {
