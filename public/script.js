...
document.addEventListener('DOMContentLoaded', function () {
    ...
    loadPricingPlans(); // Load pricing plans dynamically
...
async function loadPricingPlans() {
    try {
        ...
        if (response.ok) {
            ...
            console.error('Failed to load pricing plans:', response.status);
            // Keep hardcoded pricing plans as fallback
        }
    } catch (error) {
...
function renderPricingPlans(plans) {
    ...
    if (plans.length === 0) {
        // Keep hardcoded pricing plans
        return;
    }

    // Sort plans by orderIndex if available
...
function createPricingPlanElement(plan) {
    ...
    // Add featured class if plan is marked as popular
    if (plan.isPopular) {
        div.classList.add('featured');
    }

    // Determine WhatsApp message type based on button type
    const messageType = plan.buttonType || 'general';

    // Create features list
    const featuresList = plan.features.map(feature => `<li>${feature}</li>`).join('');

    div.innerHTML = `
        <div class="batch-badge">${plan.badge || 'Small Batch'}</div>
        <h3>${plan.title}</h3>
        <div class="price">${plan.price}</div>
        <ul class="features">
            ${featuresList}
        </ul>
        <button class="btn btn-whatsapp" onclick="openWhatsApp('${messageType}')" style="width: 100%; margin-top: 20px;">💬 ${plan.buttonText}</button>
    `;
